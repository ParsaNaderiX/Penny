"""Train JointDiT (DiT backbone) on crypto LOB data.

Same joint objective as JointDiffusion (Deja et al., 2023), with a Diffusion
Transformer backbone instead of a U-Net:
...
def main() -> None:
    ...
    ckpt_dir = (
        ...
        / f"jointdit_{config['symbol']}_{config.get('feature_mode', '')}_{stamp}"
    )
