"""Train TLOB on all symbols of one exchange (pooled, no asset conditioning).

Usage::

    uv run python -m crypto.train_multi_tlob configs/crypto/binance/multi_tlob/all_ofi.json
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "config", nargs="?", default="configs/crypto/binance/multi_tlob/all_ofi.json"
    )
    ...
    ckpt_dir = (
        ...
        / f"multi_tlob_{config.get('feature_mode', '')}_{stamp}"
    )
