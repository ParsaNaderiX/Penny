"""Train LOBTransformer on crypto LOB data.

Usage::

    uv run python -m crypto.train_lobtransformer configs/crypto/binance/lobtransformer/btcusdt_ofi.json
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "config",
        nargs="?",
        default="configs/crypto/binance/lobtransformer/btcusdt_ofi.json",
    )
    ...
    ckpt_dir = (
        ...
        / f"lobtransformer_{config['symbol']}_{config.get('feature_mode', '')}_{stamp}"
    )
