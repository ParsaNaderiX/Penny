"""Train JumpGate-DSM: the JumpGate machinery on a generalized-score objective.

The **score-based** counterpart of ``crypto.train_jumpgate_unet``.  It keeps every
JumpGate piece (the supervised noise-state estimator ``g_phi``, W-conditioning,
gated experts, W-aware trend gate) but swaps epsilon-prediction for the weighted
denoising-score-matching loss of ``crypto.train_jointdiff_levy``:

    L_diff = wbar_t * || s_hat - grad log q(x_t|x_0) ||^2      (weighted score MSE)
...
where ``wbar_t = E[W_t]`` normalizes the ~``1/W`` score magnitude to O(1) across
timesteps, and the *generalized* score target ``grad log q`` is the tabulated Lévy
score (``levy.diffusion``).  Unlike the eps trainer, the generalized-score table is
actually built (``levy_table_num_r`` / ``levy_table_mc``).

``(x_t, W, jump_flag)`` come from ``fp.add_noise_eps`` (so g_phi still gets its
supervision), while the DSM target is ``fp.score_target(x_t, x0, t)`` on the same
``x_t`` — the two are consistent draws from one forward kernel.

Ablation flags (``w_conditioning``, ``gated_experts``, ``soft_cls_gate``,
``gate_grad``) behave exactly as in the eps trainer.  ``--process gaussian`` swaps
the noising kernel + score target to the Gaussian bypass.

Usage::

    uv run python -m crypto.train_jumpgate_dsm configs/crypto/nobitex/jumpgatedsm/btcirt_ofi_k10.json
    uv run python -m crypto.train_jumpgate_dsm ... --process gaussian   # ablation
"""
...
from models.jumpgatedsm import JumpGateDSM, count_parameters
...
def _diffusion_cfg(config: dict) -> DiffusionConfig:
    ...
    Unlike ``train_jumpgate_unet`` (which bypasses the table for eps-prediction),
    the DSM objective needs the real generalized-score table, so the table sizes
    come from the config (``levy_table_num_r`` / ``levy_table_mc``).
    """
...
def _mean_W(fp: ForwardProcess, t: torch.Tensor) -> torch.Tensor:
    ...
    The per-sample DSM weight, so the weighted score target is O(1) at every
    timestep (raw score magnitude ~1/W).  Mirrors ``train_jointdiff_levy._mean_W``.
    """
...
def _train_epoch(model, fp, loader, optimizer, lr_sched, config, device):
    ...
    for batch in loader:
        ...
        # add_noise_eps gives (x_t, eps, W, jump_flag) so g_phi keeps its supervision;
        # the DSM target is the generalized score at that same x_t.
        x_t, _, W, jump_flag = fp.add_noise_eps(x0, t)
        target = fp.score_target(x_t, x0, t)
        logW = torch.log(W.clamp_min(1e-12))  # (B,)
        s_hat, logits, logW_hat, pi_logit = model(
...
def main() -> None:
    ...
    parser.add_argument(
        "config",
        nargs="?",
        default="configs/crypto/nobitex/jumpgatedsm/btcirt_ofi_k10.json",
    )
    parser.add_argument(
        "--process",
    ...
    ckpt_dir = (
        ...
        / f"jumpgatedsm_{process}_{config['symbol']}_{config.get('feature_mode', '')}_{stamp}"
    )
    ...
    logger.info(
        "JumpGateDSM  symbol={}  mode={}  process={}  schedule={}",
        config["symbol"],
    ...
    model = JumpGateDSM(config).to(device)
