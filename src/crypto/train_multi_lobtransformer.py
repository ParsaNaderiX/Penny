"""Train LOBTransformer on all symbols of one exchange (pooled, no asset conditioning).

Usage::

    uv run python -m crypto.train_multi_lobtransformer configs/crypto/binance/multi_lobtransformer/all_ofi.json
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "config",
        nargs="?",
        default="configs/crypto/binance/multi_lobtransformer/all_ofi.json",
    )
    ...
    ckpt_dir = (
        ...
        / f"multi_lobt_{config.get('feature_mode', '')}_{stamp}"
    )
    ...
    logger.info(
        "LOBTransformer [multi]  exchange={}  mode={}  n_assets={}",
        config.get("exchange"),
