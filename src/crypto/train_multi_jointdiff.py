"""Train JointDiffusion on all symbols of one exchange (pooled, no asset conditioning).

Usage::

    uv run python -m crypto.train_multi_jointdiff configs/crypto/binance/multi_jointdiff/all_ofi.json
"""
...
def _train_epoch(model, sched, loader, optimizer, lr_sched, config, device):
    ...
    for batch in loader:
        ...
        w = (1.0 - t.float() / t_max) ** 2 if trend_taper else 1.0
...
def main() -> None:
    ...
    parser.add_argument(
        "config",
        nargs="?",
        default="configs/crypto/binance/multi_jointdiff/all_ofi.json",
    )
    ...
    ckpt_dir = (
        ...
        / f"multi_jd_{config.get('feature_mode', '')}_{stamp}"
    )
    ...
    logger.info(
        "JointDiffusion [multi]  exchange={}  mode={}  n_assets={}",
        config.get("exchange"),
