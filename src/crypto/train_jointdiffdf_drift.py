"""Train JointDiffusionDF with *Drift* (Diffusion Forcing + Generative Drifting).

Same drift objective as ``train_jointdiff_drift`` (one-step generator whose output
distribution drifts toward the data distribution, Lambert et al.), but the
backbone is the Diffusion-Forcing variant: every LOB timestep carries its *own*
noise level, so the consistency map takes a **per-timestep** sigma ``(B, T)``.

For one-step generation and for the clean classifier pass every timestep is set to
the same level (``sigma_max`` / ``sigma_min`` respectively), so the generator and
predict contracts are identical to the non-DF drift trainer::

    x_gen = f_theta(sigma_max * z, sigma_max[:, T])           # noise -> LOB window
    L     = drift_loss(phi(x_gen), phi(x_pos), phi(x_neg)) + lambda_trend * CE

Usage::

    uv run python -m crypto.train_jointdiffdf_drift configs/crypto/nobitex/jointdiffdf/btcirt_ofi_k10.json
"""
...
def _train_epoch(model, loader, optimizer, lr_sched, config, device, drift, banks):
    ...
    for batch in loader:
        ...
        # One-step generation: per-timestep sigma all set to sigma_max.
        z = torch.randn_like(x0)
        sig_hi = torch.full((b, T), sigma_max, device=device)
        ...
        # Trend head on clean real windows (per-timestep sigma_min everywhere).
        sig_lo = torch.full((b, T), sigma_min, device=device)
...
def main() -> None:
    ...
    ckpt_dir = (
        ...
        / f"jointdiffdf_drift_{config['symbol']}_{config.get('feature_mode', '')}_{stamp}"
    )
    ...
    logger.info(
        "JointDiffusionDF (drift)  symbol={}  mode={}",
        config["symbol"],
