"""Train JointDiffCFG (Classifier-Free Guidance) on LOBSTER multi-asset streams.

Usage::
    uv run python src/stocks/feishu/train_jointdiffcfg_lobster.py --config configs/stocks/feishu/penny_lobster_ofi.json
"""
...
# Dynamic root path injection inside Python environment to support deep execution
...
# Secure system workspace absolute imports mapped to src
from src.models.ddpm import DDPMScheduler
from src.models.jointdiffcfg import JointDiffCFG, count_parameters
from src.stocks.feishu.build_lobster import build_datasets_multi, discover_symbols
from src.utils.evaluate import run_test
...
def main() -> None:
    ...
    parser.add_argument(
        "--config", nargs="?", default="configs/stocks/feishu/penny_lobster_ofi.json"
    )
    ...
    ckpt_dir = Path(config["checkpoint_dir"]) / f"penny_lobster_ofi_{stamp}"
    ...
    # Pack parameters from multi-asset stream dictionary
